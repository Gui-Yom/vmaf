...
#pragma unroll
            for (int yf=0; yf < filter_width_d; ++yf) {
                auto ptr = (const uint8_t*)(src.data[0]) + mirror(y-radius+yf, height) * src.stride[0];
                blurred_y += filter_d[yf] * ptr[mirror(x-radius+xf, width)];
            }
...
#pragma unroll
            for (int yf=0; yf < filter_width_d; ++yf) {
            auto ptr = (const uint16_t*)(src.data[0]) + mirror(y-radius+yf, height) * src.stride[0];
                blurred_y += filter_d[yf] * ptr[mirror(x-radius+xf, width)];
            }
            blurred += filter_d[xf]*((blurred_y + add_before_shift_y) >> shift_var_y);
        }
